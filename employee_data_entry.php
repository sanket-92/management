    ...
    <body>
        ...
        <div class="container">
            ...
            <form class="form-signin" action="insert.php" method="POST">
                ...
                <input <?php if ($_SESSION["type"] == "edit") {
    echo "readonly";
} ?> name="emp" type="text" onkeyup="emp_check()" pattern="[0-9]+"  oninvalid="setCustomValidity('Plz Enter Valid Employee ID ')"
                                                                                  onclick="emp_check()" value='<?php echo $_SESSION['id']; ?>' onchange="try {
                                                                                              setCustomValidity('')
                                                                                          } catch (e) {
                                                                                          }" class="emp"  id="emp" placeholder="Employee ID" maxlength="10" required placeholder autocomplete="off"><span id="emp-result" autocomplete="off"></span><br>
                <p>Enter Your Name</p>
                <input name="name" type="text" pattern="[a-zA-Z ]+" oninvalid="setCustomValidity('Plz Enter Valid Name')"
                       onkeyup="name_check()" value='<?php echo $_SESSION['name']; ?>' onchange="try {
                                   setCustomValidity('')
                               } catch (e) {
                               }" class="name" id="name" placeholder="Name" maxlength="20" required autocomplete="off"><span id="name-result" autocomplete="off"></span><br>
                <p>Age</p>
                <input name="age" type="text"  pattern="[0-9]+" oninvalid="setCustomValidity('Plz Enter Valid Age')"
                       onchange="try {
                                   setCustomValidity('')
                               } catch (e) {
                               }" value='<?php echo $_SESSION['age']; ?>' class="age" id="age" onkeyup="age_check()" placeholder="Age" maxlength="3" required autocomplete="off"><span id="age-result" autocomplete="off"></span>
                <br>
                <p>Email ID</p>
                <input name="email" type="text"  pattern="[a-zA-Z0-9\._-]+@[a-zA-Z]+\.[a-zA-Z]{2,6}" oninvalid="setCustomValidity('Plz Enter Valid Email ID')"
                       onchange="try {
                               ...
                               }" onkeyup="email_check()" value='<?php echo $_SESSION['email']; ?>'  class="email"  id="email" age="email" maxlength="20"  placeholder="Email ID" required autocomplete="off"><span id="email-result" autocomplete="off"></span><br>
                <br>
